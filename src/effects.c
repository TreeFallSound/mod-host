...
static lv2_index_t *g_lv2_index;
...
#ifdef HAVE_NEW_LILV
/* bundlepath must be absolute and end in a separator. Loading a bundle that is
   already in the world is a no-op. */
static void LoadBundleByPath(const char *bundlepath)
{
...
/* Puts every bundle the plugin needs into the world: its own, plus any holding
   presets that apply to it. Must run before the world is queried, because
   LoadPresets() resolves presets out of the world at instantiation time. */
static void LoadBundlesForPlugin(const char *uri)
{
    const char *const *bundles = lv2_index_bundles_for_plugin(g_lv2_index, uri);

    if (bundles == NULL)
        return;

    for (; *bundles != NULL; ++bundles)
        LoadBundleByPath(*bundles);

    lv2_index_mark_plugin_loaded(g_lv2_index, uri);
    g_plugins = lilv_world_get_all_plugins(g_lv2_data);
}

static void LoadBundleForResource(const char *uri)
{
    const char *bundle = lv2_index_bundle_for_resource(g_lv2_index, uri);

    if (bundle != NULL)
        LoadBundleByPath(bundle);
}
#endif
...
int effects_init(void* client)
{
    ...
    /* Index the LV2 bundles, but load as few of them as possible.
       lilv_world_load_all parses every manifest on LV2_PATH into a single sord
       model, which on a board with hundreds of plugins costs most of our
       startup time and a large chunk of RAM -- for data mod-host never reads.
       Instead we skim the manifests (see lv2_index.c), load only the bundles
       that must be resident up front, and pull the rest in from effects_add as
       plugins are actually instantiated. */
    g_lv2_data = lilv_world_new();
    g_lv2_index = lv2_index_build();

#ifdef HAVE_NEW_LILV
    if (g_lv2_index != NULL)
    {
        const char *const *eager = lv2_index_eager_bundles(g_lv2_index);

        if (eager != NULL)
        {
            for (; *eager != NULL; ++eager)
                LoadBundleByPath(*eager);
        }

        /* Specifications carry things the plugin bundles refer to but do not
           define, such as the rdfs:range of a patch property. Classes must come
           after them. */
        lilv_world_load_specifications(g_lv2_data);
        lilv_world_load_plugin_classes(g_lv2_data);

        unsigned bundles, plugins, resources, specs;
        lv2_index_stats(g_lv2_index, &bundles, &plugins, &resources, &specs);
        fprintf(stdout, "mod-host: indexed %u LV2 bundles (%u plugins, %u presets), "
                        "loaded %u eagerly\n", bundles, plugins, resources, specs);
    }
    else
#endif
    {
...
int effects_add(const char *uri, int instance)
{
    ...
    /* Get the plugin */
#ifdef HAVE_NEW_LILV
    /* A plugin generated by a dyn-manifest is not in the index, but its bundle
       was loaded eagerly, so a miss here is not an error: let the world decide. */
    LoadBundlesForPlugin(uri);
#endif
...
int effects_preset_load(int effect_id, const char *uri)
{
    ...
    {
#ifdef HAVE_NEW_LILV
        /* Normally redundant -- adding the effect loaded its preset bundles --
           but covers a preset bundle installed since then. */
        LoadBundleForResource(uri);
#endif
...
int effects_preset_save(int effect_id, const char *dir, const char *file_name, const char *label)
{
    ...
    /* lilv_state_save writes a manifest into dir but does not load it. mod-ui
       normally follows up with a bundle_add; index it here too so the preset is
       still findable if it does not. */
    if (ret == 0)
        lv2_index_add_bundle(g_lv2_index, dir);
...
{
#ifdef HAVE_NEW_LILV
    /* No effect id here, so nothing else will have loaded the bundle this
       preset lives in. */
    LoadBundleForResource(uri);
#endif
...
void effects_bundle_add(const char* bpath)
{
    ...
    // keep the index in step: a preset bundle for a plugin that is not loaded
    // yet still has to be found when that plugin is eventually added
    lv2_index_add_bundle(g_lv2_index, bundlepath);
#else
    UNUSED_PARAM(bpath);
#endif
}
...
void effects_bundle_remove(const char *bpath, const char *resource)
{
    ...
    lv2_index_remove_bundle(g_lv2_index, bundlepath);
#else
    UNUSED_PARAM(bpath);
#endif
}
